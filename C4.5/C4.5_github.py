...
def getLabelsAndLabelType(dataset):  # 获取标签和标签类型
    # 获取标签并删除
    labels = dataset[0]
    del (dataset[0])
    ...
    for i in range(len(labels) - 1):
        if isinstance(dataset[1][i], int):
            labelType.append('continuous')
        else:
            labelType.append('uncontinuous')
...
def createDateset(filename):
    with open(filename, 'r', encoding='utf-8') as csvfile:
        dataset = [line.strip().split(',')
                   for line in csvfile.readlines()]  # 读取文件中的每一行

        # 将数据集从字符串列表转换为整数列表列表
        # 对于每一行中的每一个元素，将行列式数字化并且去除空白保证匹配的正确完成
        # dataset = [[int(i) if i.isdigit() else i for i in row] for row in dataset]
        cleanoutdata(dataset)  # 清洗数据
        dataset = [[str2num(i) for i in row] for row in dataset]

        # 去除最后一行的空行
        # del (dataset[-1])  # 这好像没有空行
        ...
        labels = dataset[0]
        del (dataset[0])
        # labels, labelType = getLabelsAndLabelType(dataset)
        # 1, 1, 81, 41, 250, 1.98, 6.1, 9.0, 1, 12.0, 11.11, 2.44
        labelType = ['uncontinuous', 'uncontinuous', 'continuous', 'continuous', 'continuous', 'continuous', 'continuous',
...
def splitContinuousDataSet(dataSet, feature, continuousValue):  # 根据连续值来划分数据集
    ...
    for vec in dataSet:
        ...
        # 如果feature列的值比最佳分割点的大，则放在biggerDataSet中，否则放在smallerDataSet中
        if vec[feature] > continuousValue:
...
def addFeatureValue(featureListOfValue, feature):
    feat = [['Private', 'Self-emp-not-inc', 'Self-emp-inc',
            ...
            [], [], [], [], []]
    # feat保存的是所有属性特征的所有可能的取值，其结构为feat = [ [val1,val2,val3,…,valn], [], [], [], … ,[] ]
    for featureValue in feat[feature]:
...
def calGainRatioUnContinuous(dataSet, feature, HC):
    ...
    # 对于dataset中每一个feature，创建单独的列表list保存其取值，其中是不重复的
    featureListOfValue = [vector[feature] for vector in dataSet]
    ...
    for value in unique:
        ...
        # 计算当前类别的香农熵，并和HTC想加，即H(T|C) = H（T1|C）+ H(T2|C) + … + H(TN|C)
        HTC += probability * calculateEntropy(subDataSet)
...
def calGainRatioContinuous(dataSet, feature, HC):
    ...
    for i in range(len(splitPoint)):
        HTC = 0.0
        split = 0.0
        biggerDataSet, smallerDataSet = splitContinuousDataSet(
            dataSet, feature, splitPoint[i])
...
def chooseBestSplitWay(dataSet, labelType):
    ...
    # 计算整个数据集的香农熵(期望信息)，即H(C)，用来和每个feature的香农熵进行比较
    HC = calculateEntropy(dataSet)
    bestfeatureIndex = -1  # 最好的划分方式的索引值，因为0也是索引值，所以应该设置为负数
    # 信息增益率=(期望信息-熵)/分割获得的信息增益，即为GR = IG / split = ( HC - HTC )/ split , gainRatioMax为最好的信息增益率，IG为各种划分方式的信息增益
    GRMAX = 0.0
    continuousValue = -1  # 设置如果是连续值的属性返回的最好的划分方式的最好分割点的值
    # 计算feature的个数，由于dataset中是包含有类别的，所以要减去类别
...
def majority(classList):
    ...
    sortedClassification = sorted(
        dict2list(classificationCount), key=operator.itemgetter(1), reverse=True)
...
def createTree(dataSet, labels, labelType):
    classificationList = [feature[-1]
                          for feature in dataSet]  # 产生数据集中的分类列表，保存的是每一行的分类
    # pprint_and_exit(classificationList)
    # 如果分类别表中的所有分类都是一样的，则直接返回当前的分类
    ...
    # 计算香农熵和信息增益来返回最佳的划分方案，bestFeature保存最佳的划分的feature的索引，在C4.5中要判断该feature是连续型还是离散型的,continuousValue是当前的返回feature是continuous的的时候，选择的“最好的”分割点
    bestFeature, continuousValue, isContinuous = chooseBestSplitWay(
        dataSet, labelType)
    ...
    if isContinuous == 1:  # 如果要划分的feature是连续的
        ...
        biggerDataSet, smallerDataSet = splitContinuousDataSet(
            dataSet, bestFeature, continuousValue)  # 根据最佳分割点将数据集划分为两部分，一个是大于最佳分割值，一个是小于等于最佳分割值
        subLabels = labels[:]
        subLabelType = labelType[:]
        Tree[bestFeatureLabel]['>'+str(continuousValue)] = createTree(
            biggerDataSet, subLabels, subLabelType)  # 将大于分割值的数据集加入到树中，并且递归创建这颗子树
        subLabels = labels[:]
        subLabelType = labelType[:]
        Tree[bestFeatureLabel]['<'+str(continuousValue)] = createTree(
            smallerDataSet, subLabels, subLabelType)  # 将小于等于分割值的数据集加入到树中，并且递归创建这颗子树
    else:  # 如果要划分的feature是非连续的，下面的步骤和ID3决策树相同
        # 构造以当前的feature作为root节点，它的所有存在的feature取值为叶子的子树
        # 对于上述取出的bestFeature,取出数据集中属于当前feature的列的所有的值
        featureValueList = [feature[bestFeature]for feature in dataSet]
        ...
        for value in uniqueValue:  # 对于每一个feature标签的value值，进行递归构造决策树
            ...
            Tree[bestFeatureLabel][value] = createTree(splitDataSet(
                dataSet, bestFeature, value), subLabels, subLabelType)
    return Tree
...
def classify(inputTree, featLabels, testVector, labelType):
    ...
    if labelType[featIndex] == 'uncontinuous':  # 如果是离散型的标签，则按照ID3决策树的方法来测试算法
        for key in dictionary.keys():  # 对于这个字典
            if testVector[featIndex] == key:
                if type(dictionary[key]).__name__ == 'dict':  # 如果还有一个新的字典
                    # 递归向下寻找到非字典的情况，此时是叶子节点，叶子节点保存的肯定是类别
                    classLabel = classify(
                        dictionary[key], featLabels, testVector, labelType)
                else:
        ...
        if float(testVector[featIndex]) > float(number):  # 如果测试向量是>n的情况
            # 设置一个判断string，它是firstBranch的还原，但是为了节省判断branch是哪一种，直接使用字符串连接的方式链接
            string = '>'+str(number)
        else:
            # 设置一个判断string，它是firstBranch的还原，但是为了节省判断branch是哪一种，直接使用字符串连接的方式链接
            string = "<="+str(number)
...
def test(mytree, labels, filename, labelType, mydate):
    with open(filename, 'r')as csvfile:
        dataset = [line.strip().split(', ')
                   for line in csvfile.readlines()]  # 读取文件中的每一行
        dataset = [[int(i) if i.isdigit() else i for i in row]
                   for row in dataset]  # 对于每一行中的每一个元素，将行列式数字化并且去除空白保证匹配的正确完成
...
def main():
    ...
    dataSetName = 'DATASETS_CorkStoppers.csv'
    ...
    # pprint_and_exit(mydate, label, labelType)
    print(Tree)
    ...
    total, correct, error = test(
        Tree, label, r'C:\Users\yang\Desktop\adult.test', labelType, mydate)
