...
class DetectionNetwork(object):
    ...
    def add_anchor_img_smry(self, img, anchors, labels):
        ...
        anchors_in_img=show_box_in_tensor.only_draw_boxes(img_batch=img,
                                                        boxes=negative_anchor)
        return anchors_in_img
    ...
    def add_roi_batch_img_smry(self, img, rois, labels):
        ...
        positive_label = tf.gather(labels, positive_roi_indices)
        positive_label = tf.reshape(positive_label, [-1])
        pos_roi = tf.gather(rois, positive_roi_indices)

        pos_in_img = show_box_in_tensor.draw_boxes_with_categories(img_batch=img,
                                                                   boxes=pos_roi, labels=positive_label)
            ...
            with tf.variable_scope('FastRCNN_loss'):
                if not cfgs.FAST_RCNN_MINIBATCH_SIZE == -1:
                    ...
                    cls_loss, bbox_loss = losses.sum_ohem_loss(cls_score=cls_score,
                                                               label=labels,
                                                               bbox_tadd_anchor_img_smryargets=bbox_targets,
                                                               bbox_pred=bbox_pred,
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        if self.is_training:
            with tf.variable_scope('sample_anchors_minibatch'):
                ...
                rpn_labels = tf.reshape(rpn_labels, [-1])
                all_anchors=self.add_anchor_img_smry(input_img_batch, anchors, rpn_labels)
            ...
            with tf.control_dependencies([rpn_labels]):
                with tf.variable_scope('sample_RCNN_minibatch'):
                    ...
                    pos_in_img=self.add_roi_batch_img_smry(input_img_batch, rois, labels)

        # -------------------------------------------------------------------------------------------------------------#
        #                                            Fast-RCNN                                                         #
        # -------------------------------------------------------------------------------------------------------------#

        return all_anchors,pos_in_img
